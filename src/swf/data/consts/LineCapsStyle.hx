...
class LineCapsStyle
{
	...
	public static function toString(lineCapsStyle:Int):String
	{
		...
		{
			case ROUND:
				return Std.string(CapsStyle.ROUND).toLowerCase();
			case NO:
				return Std.string(CapsStyle.NONE).toLowerCase();
			case SQUARE:
				return Std.string(CapsStyle.SQUARE).toLowerCase();
			default:
				return "unknown";
		}
	}
