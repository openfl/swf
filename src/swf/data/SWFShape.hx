...
import swf.SWFData;
import swf.data.consts.GradientInterpolationMode;
import swf.data.consts.GradientSpreadMode;
import swf.data.consts.LineCapsStyle;
import swf.data.consts.LineJointStyle;
import swf.data.etc.CurvedEdge;
import swf.data.etc.IEdge;
import swf.data.etc.StraightEdge;
import swf.exporters.core.DefaultShapeExporter;
import swf.exporters.core.IShapeExporter;
import swf.utils.ColorUtils;
import swf.utils.NumberUtils;
...
class SWFShape
{
	...
	public function export(handler:IShapeExporter = null):Void
	{
		...
		if (handler == null)
		{
			handler = new DefaultShapeExporter(null);
		}
						...
						if (fillStyle != null)
						{
							switch (fillStyle.type)
							{
								...
								case 0x10, 0x12, 0x13:
									...
									handler.beginGradientFill((fillStyle.type == 0x10) ? GradientType.LINEAR : GradientType.RADIAL, colors, alphas, ratios,
										matrix, GradientSpreadMode.toEnum(fillStyle.gradient.spreadMode),
										GradientInterpolationMode.toEnum(fillStyle.gradient.interpolationMode), fillStyle.gradient.focalPoint);
						...
						if (lineStyle.hasFillFlag)
						{
							...
							switch (fillStyle.type)
							{
								case 0x10, 0x12, 0x13:
									...
									handler.lineGradientStyle((fillStyle.type == 0x10) ? GradientType.LINEAR : GradientType.RADIAL, colors, alphas, ratios,
										matrix, GradientSpreadMode.toEnum(fillStyle.gradient.spreadMode),
										GradientInterpolationMode.toEnum(fillStyle.gradient.interpolationMode), fillStyle.gradient.focalPoint);
							}
						}
