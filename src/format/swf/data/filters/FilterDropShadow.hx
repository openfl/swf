...
class FilterDropShadow extends Filter implements IFilter
{
	...
	{
		return new DropShadowFilter(distance, angle * 180 / Math.PI, ColorUtils.rgb(dropShadowColor), ColorUtils.alpha(dropShadowColor), blurX, blurY,
			strength, passes, innerShadow, knockout);
	}

	override private function get_type():FilterType
	{
		return DropShadowFilter(distance, angle * 180 / Math.PI, ColorUtils.rgb(dropShadowColor), ColorUtils.alpha(dropShadowColor), blurX, blurY, strength,
			passes, innerShadow, knockout, false);
	}
