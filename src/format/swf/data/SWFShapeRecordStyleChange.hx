...
class SWFShapeRecordStyleChange extends SWFShapeRecord
{
	...
	override public function publish(data:SWFData = null, level:Int = 1):Void
	{
		...
		if (stateFillStyle0)
		{
			data.writeUB(numFillBits, fillStyle0);
		}
		if (stateFillStyle1)
		{
			data.writeUB(numFillBits, fillStyle1);
		}
		if (stateLineStyle)
		{
			data.writeUB(numLineBits, lineStyle);
		}
		if (stateNewStyles)
		{
			data.resetBitsPending();
	...
	override public function toString(indent:Int = 0):String
	{
		...
		if (stateMoveTo)
		{
			cmds.push("MoveTo: " + moveDeltaX + "," + moveDeltaY);
		}
		if (stateFillStyle0)
		{
			cmds.push("FillStyle0: " + fillStyle0);
		}
		if (stateFillStyle1)
		{
			cmds.push("FillStyle1: " + fillStyle1);
		}
		if (stateLineStyle)
		{
			cmds.push("LineStyle: " + lineStyle);
		}
		if (cmds.length > 0)
		{
			str += cmds.join(", ");
